...
let seq = 100;
function task(project, section, status, tag, title, desc, who, links, comments, due, priority) {
...
let SECTOR_MEMBERS = {};
let REQUIRED_SECTOR_MEMBERS = {};
function sectorPeople(extra = []) {
...
function $(sel, root = document) {
    ...
    if (!el)
        throw new Error("elemento não encontrado: " + sel);
    return el;
}
const esc = (s) => String(s).replace(/[&<>"']/g, c => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[c]));
function project(id = state.project) {
...
const tagChip = (name) => `<span class="tag" style="--h:${TAGS[name] ?? 0}">${esc(name)}</span>`;
const dayDiff = (iso) => Math.round((new Date(iso + "T00:00:00").getTime() - TODAY.getTime()) / 86400000);
...
const isLate = (t) => t.status !== "done" && dayDiff(t.due) < 0;
...
const prioChip = (p) => `<span class="prio ${p}" title="Prioridade ${PRIO_NAME[p].toLowerCase()}">${I[p]}${PRIO_NAME[p]}</span>`;
/* ============================ filtros ============================ */
const weekEnd = () => {
...
const inSection = (t) => t.project === state.project && t.section === state.section;
/** A tarefa está agendada para este dia da semana na linha do tempo? */
const scheduledOn = (t, day) => SCHEDULE.some(b => b.day === day && b.task === t.title);
...
function isMine(t, range = state.mineRange) {
    if (!t.who.includes(ME))
        return false;
    if (range === "tudo")
        return true;
    const d = dayDiff(t.due);
    ...
    if (range === "hoje")
        return scheduledOn(t, weekdayOf(TODAY)) || d === 0 || (open && d < 0);
    return scheduledOn(t, 0) || scheduledOn(t, 1) || scheduledOn(t, 2) || scheduledOn(t, 3) || scheduledOn(t, 4)
        || (d >= 0 && d <= 6) || (open && d < 0);
}
function isVisible(t) {
    if (state.mine ? !isMine(t) : !state.query && !inSection(t))
        return false;
    if (state.tags.size && !state.tags.has(t.tag))
        return false;
    if (state.priorities.size && !state.priorities.has(t.priority))
        return false;
    if (state.who.size && !t.who.some(k => state.who.has(k)))
        return false;
    if (state.weekOn) {
        const d = new Date(t.due + "T00:00:00");
        if (d < state.week || d > weekEnd())
            return false;
    }
    if (!matchesSearch(t))
        return false;
...
const filterCount = () => state.tags.size + state.who.size + state.priorities.size + (state.query ? 1 : 0) + (state.weekOn ? 1 : 0);
/* ============================ barra lateral ============================ */
function projectRow(p, expand, colored) {
    const active = p.id === state.project;
    const cls = `pitem${colored ? " hue" : ""}`;
    let html = `<button class="${cls}" data-project="${p.id}" style="--dh:${p.hue}"${active ? ' aria-current="true"' : ""}>` +
...
function groupBlock(key, label, items, colored, expand) {
...
function renderSidebar() {
    ...
    let html = `<button class="mine" data-mine="1"${state.mine ? ' aria-current="true"' : ""}>` +
    ...
    if (popBridgeActive) {
        ...
        if (favoriteSections.length) {
            const rows = favoriteSections.map(section => `<button class="pitem hue" data-section="${section.id}" style="--dh:${project().hue}"` +
...
function renderHeader() {
    ...
    $("#headLeft").innerHTML =
        `<span class="tree" aria-hidden="true"></span>` +
            ...
            `<div class="tabs">` + p.sections.map(x => `<button class="tab" data-section="${x.id}"${x.id === state.section ? ' aria-current="true"' : ""}>` +
...
function renderMineHeader() {
    const ranges = [["hoje", "Hoje"], ["semana", "Esta semana"], ["tudo", "Tudo"]];
    ...
    const legenda = state.mineRange === "hoje" ? "O que está na sua agenda de hoje, mais o que vence hoje e o que já passou do prazo."
    ...
    $("#brief").innerHTML =
        `<div class="brief-h"><h2>${state.mineRange === "hoje" ? "Para fazer hoje"
            ...
            `<div class="rangeseg" style="margin-left:auto">` + ranges.map(([k, l]) => `<button data-minerange="${k}"${state.mineRange === k ? ' aria-current="true"' : ""}>${l}</button>`).join("") +
            `</div></div>` +
            `<p>${legenda}</p>` +
            `<div class="brief-b"><div class="meta">` +
...
function renderList(list) {
    return `<div class="list">` + STATUS.map(s => {
        ...
        if (!items.length)
            return "";
        const rows = items.map((t, i) => `<button class="lrow" data-id="${t.id}" style="--i:${Math.min(i, 10)}">` +
...
function renderView() {
    document.querySelectorAll("#viewSeg button").forEach(button => button.setAttribute("aria-current", String(button.dataset["view"] === state.view)));
    const view = $("#view");
    const list = visibleTasks();
    if (state.view === "kanban") {
        view.innerHTML = renderKanban(list);
        return;
    }
    if (!list.length) {
...
function openLayer(html, onMount) {
    ...
    scrim?.addEventListener("mousedown", e => { if (e.target === scrim)
        closeLayer(); });
    onMount?.();
    layer.querySelector("[data-focus]")?.focus();
}
let toastTimer = 0;
function toast(msg) {
...
function openTaskForm(existing, presetStatus) {
    const edit = !!existing;
    const d = existing ?? {
    ...
    openLayer(`<div class="scrim"><div class="sheet" role="dialog" aria-modal="true" aria-label="${edit ? "Editar tarefa" : "Nova tarefa"}">` +
        ...
        ["alta", "media", "baixa"].map(x => `<option value="${x}"${x === d.priority ? " selected" : ""}>${PRIO_NAME[x]}</option>`).join("") + `</select></div>` +
        `</div>` +
        `<div class="two">` +
        ...
        sectorPeople(d.who).map(k => `<button type="button" class="pbtn" data-p="${k}" aria-pressed="${d.who.includes(k)}">` +
        ...
        $("#taskForm").addEventListener("submit", e => {
            ...
            const who = [...layer.querySelectorAll('.pbtn[aria-pressed="true"]')].map(b => b.dataset["p"]);
            const links = Math.max(0, parseInt($("#fL").value, 10) || 0);
            const tag = $("#fG").value.trim() || "Geral";
            if (!(tag in TAGS))
                TAGS[tag] = hueFrom(tag);
            const patch = {
                title,
                desc: $("#fD").value.trim(),
                tag,
...
function openSectionForm() {
    ...
    openLayer(`<div class="scrim"><div class="sheet" role="dialog" aria-modal="true" aria-label="Editar setor">` +
        ...
        $("#secForm").addEventListener("submit", e => {
            e.preventDefault();
            const nextName = $("#sN").value.trim();
            const nextDescription = $("#sD").value.trim();
            if (!nextName) {
                $("#sN").focus();
                return;
            }
            ...
            s.hours = $("#sH").value.trim() || s.hours;
            const parts = $("#sR").value.split(/[—–]/);
            if (parts.length > 1) {
                s.from = parts[0].trim();
                s.to = parts.slice(1).join("–").trim();
            }
            popPost("sector:update", { sectionId: s.id, name: s.name, description: s.desc });
            closeLayer();
            renderAll();
            toast("Setor atualizado e salvo.");
        });
    });
}
function openProjectForm() {
    openLayer(`<div class="scrim"><div class="sheet" style="width:min(480px,100%)" role="dialog" aria-modal="true" aria-label="Novo projeto">` +
        ...
        $("#projForm").addEventListener("submit", e => {
            ...
            const sid = "s" + ++seq;
            PROJECTS.push({
                id, name, favorite: false, group: $("#pC").value,
                hue: (seq * 47) % 360,
                sections: [{
                        id: sid, name: $("#pS").value.trim() || "Visão geral",
...
function openSectorMembers() {
    ...
    openLayer(`<div class="scrim"><div class="sheet" style="width:min(560px,100%)" role="dialog" aria-modal="true" aria-label="Membros do setor">` +
        ...
        `<div class="people" id="sectorMembers">` + Object.keys(PEOPLE).map(id => `<button type="button" class="pbtn" data-member="${id}" aria-pressed="${selected.has(id)}"` +
        ...
        $("#sectorMembersForm").addEventListener("submit", event => {
            event.preventDefault();
            const memberIds = [...layer.querySelectorAll('[data-member][aria-pressed="true"]')]
...
function openFilters(anchor) {
    ...
    openLayer(`<div class="scrim" style="background:transparent;backdrop-filter:none;display:block">` +
        ...
        ["alta", "media", "baixa"].map(k => `<button class="chip" data-prio="${k}" aria-pressed="${state.priorities.has(k)}">${PRIO_NAME[k]}</button>`).join("") +
        `</div><h4>Etiquetas</h4><div class="chips">` +
        Object.keys(TAGS).map(k => `<button class="chip" data-tag="${esc(k)}" aria-pressed="${state.tags.has(k)}">${esc(k)}</button>`).join("") +
        `</div><h4>Pessoas</h4><div class="chips">` +
        sectorPeople().map(k => `<button class="chip" data-who="${k}" aria-pressed="${state.who.has(k)}">${esc(PEOPLE[k].name)}</button>`).join("") +
        ...
        $(".pop", layer).addEventListener("click", e => {
            ...
            const who = el.closest("[data-who]");
            const prio = el.closest("[data-prio]");
            if (tag) {
                toggle(state.tags, tag.dataset["tag"]);
                tag.setAttribute("aria-pressed", String(state.tags.has(tag.dataset["tag"])));
            }
            else if (who) {
                toggle(state.who, who.dataset["who"]);
                who.setAttribute("aria-pressed", String(state.who.has(who.dataset["who"])));
            }
            else if (prio) {
                const v = prio.dataset["prio"];
                toggle(state.priorities, v);
                prio.setAttribute("aria-pressed", String(state.priorities.has(v)));
            }
...
function openTagManager() {
    ...
    openLayer(`<div class="scrim"><div class="sheet tag-manager" role="dialog" aria-modal="true" aria-label="Cadastrar etiqueta">` +
        ...
        $("#tagManagerForm").addEventListener("submit", e => {
            e.preventDefault();
            const input = $("#newTag");
            ...
            if (existing) {
                input.focus();
                toast(`A etiqueta “${existing}” já existe.`);
                return;
            }
            ...
            closeLayer();
            renderScreen();
            toast(`Etiqueta “${formatted}” cadastrada e salva.`);
        });
    });
}
/* ============================ arrastar e soltar ============================ */
...
document.addEventListener("dragover", e => {
    ...
    if (dt)
        dt.dropEffect = "move";
    document.querySelectorAll(".col.over").forEach(c => { if (c !== col)
...
document.addEventListener("drop", e => {
    ...
    dragId = null;
    renderScreen();
    requestAnimationFrame(() => document.querySelector(`.card[data-id="${moved}"]`)?.classList.add("flash"));
...
document.addEventListener("click", e => {
    ...
    if (add) {
        openTaskForm(undefined, add.dataset["add"]);
        return;
    }
    if (at("#newBtn")) {
        openTaskForm();
    ...
    if (at("[data-copy-section]")) {
        closeLayer();
        toast(`Link copiado: orbita.studio/p/${state.project}/${state.section}`);
        return;
    }
    ...
    if (at("#bellBtn")) {
        toast("10 novidades: 6 comentários, 3 revisões, 1 prazo hoje.");
        return;
    }
    if (at("#linkBtn")) {
...
function legend(slices) {
    return `<div class="leg">` + slices.map(s => `<span><i style="background:${s.color}"></i>${esc(s.label)} <b style="color:var(--tx)">${s.value}</b></span>`).join("") + `</div>`;
}
/** Barra empilhada horizontal: parte-todo. Legenda sempre presente. */
function stackedBar(slices, total) {
    return `<div class="sbar">` + slices.filter(s => s.value > 0).map(s => `<i style="width:${pct(s.value, total)}%;background:${s.color}" ` +
...
function columns(slices) {
    const max = Math.max(1, ...slices.map(s => s.value));
    const body = slices.map(s => `<div class="colw" data-tip="${esc(s.label)}: ${plural(s.value, "tarefa", "tarefas")}">` +
...
function hBars(rows) {
    ...
    return `<div class="hbars">` + rows.map(r => `<div class="hrow" data-tip="${esc(r.name)}: ${esc(r.label ?? String(r.value))}">` +
...
const findTask = (title) => TASKS.find(t => t.title === title);
const hhmm = (h) => `${String(Math.floor(h)).padStart(2, "0")}:${String(Math.round((h % 1) * 60)).padStart(2, "0")}`;
const clock = (secs) => [Math.floor(secs / 3600), Math.floor(secs / 60) % 60, secs % 60].map(n => String(n).padStart(2, "0")).join(":");
...
function activityCard() {
    ...
    return `<section class="dcard" style="--i:3">` +
        ...
        `<div class="abars">` + ACTIVITY.map((a, i) => `<div class="abar${i === todayIdx ? " now" : ""}" data-tip="${a.day}: ${a.value}% do dia apontado">` +
...
function workedCard() {
    ...
    return `<section class="dcard" style="--i:4">` +
        ...
        `<div class="dleg">` + WORKED.map((w, i) => `<span data-tip="${esc(project(w.project).name)}: ${w.hours}h de ${total}h"><i style="background:${cols[i]}"></i>` +
...
function remindersCard() {
    ...
    return `<section class="dcard" style="--i:5">` +
        ...
        `<div class="rems">` + list.map(r => `<div class="rem"><b>${hhmm(r.hour)}<em>${r.hour < 12 ? "AM" : "PM"}</em></b>` +
...
function monthGrid(base, onDay) {
    const first = new Date(base.getFullYear(), base.getMonth(), 1);
    const start = weekStart(first);
    const cells = ["seg", "ter", "qua", "qui", "sex", "sáb", "dom"].map(d => `<div class="dow">${d}</div>`);
    for (let i = 0; i < 42; i++) {
        const d = addDays(start, i);
        const out = d.getMonth() !== base.getMonth();
        const iso = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
        const due = TASKS.filter(t => t.due === iso);
        const dots = due.slice(0, 5).map(t => `<i style="background:${statusOf(t.status).color}"></i>`).join("");
        cells.push(`<div class="mday${out ? " out" : ""}${sameDay(d, TODAY) ? " today" : ""}" ` +
...
function rangeSeg() {
    const opts = [["hoje", "Hoje"], ["semana", "Semana"], ["mes", "Mês"], ["ano", "Ano"]];
    return `<div class="rangeseg" id="rangeSeg">` + opts.map(([k, l]) => `<button data-range="${k}"${state.range === k ? ' aria-current="true"' : ""}>${l}</button>`).join("") + `</div>`;
}
function scheduleView() {
    if (state.range === "hoje")
        return timeline([1]);
    if (state.range === "semana")
        return timeline([0, 1, 2, 3, 4]);
    if (state.range === "mes")
        return `<section class="dcard" style="--i:0">${monthGrid(TODAY)}</section>`;
...
function indicatorCards() {
    ...
    cards.push(dcard({
        ...
        chart: `<div class="kpis">` + ["alta", "media", "baixa"].map(k => {
...
const laneOf = (e) => (e.kind === "tarefa" || e.kind === "intervalo" ? 0 : 1);
function layoutEvents(list) {
    ...
    for (const lane of [0, 1]) {
        ...
        let group = [];
        let groupEnd = -1;
        const flush = () => { group.forEach((e, i) => { e.col = i; e.cols = group.length; }); group = []; };
...
function dayEvents() {
    ...
    const out = [];
    const stateOf = (start, end) => !isToday ? (d < TODAY ? { past: true } : {})
...
function eventRow(e) {
    ...
    const plat = e.kind === "tarefa" ? `<span class="plat" title="Tarefa">${I.pen}</span>`
        : e.kind === "lembrete" ? `<span class="plat" title="Lembrete">${I.clock}</span>` : brk ? "" :
            `<span class="plat" title="${KIND_NAME[e.kind]}">${KIND_ICON[e.kind]}</span>`;
...
function hasEvents(d) {
    const iso = isoOf(d);
    if (MEETINGS.some(m => m.date === iso) || REMINDERS.some(r => r.date === iso))
        return true;
    if (TASKS.some(t => t.due === iso))
        return true;
...
function teamList() {
    ...
    return `<div><h3>Time no projeto</h3><div class="team">` +
        Object.entries(PEOPLE).slice(0, 5).map(([k, p], i) => `<button class="tmrow" data-chat="${k}">` +
...
function openMeeting(id) {
    ...
    openLayer(`<div class="scrim"><div class="sheet" style="width:min(520px,100%)" role="dialog" aria-modal="true" ` +
...
function openMeetingForm(existing) {
    ...
    const kinds = ["video", "chamada", "onboarding", "revisao", "daily"];
    openLayer(`<div class="scrim"><div class="sheet" role="dialog" aria-modal="true" ` +
        ...
        [15, 30, 45, 60, 90, 120].map(v => `<option value="${v}"${Math.round((m.end - m.hour) * 60) === v ? " selected" : ""}>${v} min</option>`).join("") +
        `</select></div>` +
        `<div class="field"><label>Participantes</label><div class="people" id="mW">` +
        Object.keys(PEOPLE).map(k => `<button type="button" class="pbtn" data-p="${k}" aria-pressed="${m.who.includes(k)}">` +
            `${avatar(k)}${esc(PEOPLE[k].name)}</button>`).join("") +
        `</div></div>` +
        `<div class="acts">` +
        ...
        $("#meetForm").addEventListener("submit", e => {
            ...
            const [hh, mm] = $("#mH").value.split(":").map(Number);
            const hour = (hh ?? 14) + (mm ?? 0) / 60;
            const dur = Number($("#mDur").value) / 60;
            const who = [...layer.querySelectorAll('.pbtn[aria-pressed="true"]')].map(b => b.dataset["p"]);
            const patch = {
                title, desc: $("#mD").value.trim(),
                kind: $("#mK").value,
                date: $("#mDate").value || m.date,
            ...
            if (existing) {
                ...
                if (state.calFilter === "tarefas")
                    state.calFilter = "tudo";
...
function renderCalendar() {
    const d = state.calDate;
    const filters = [["tarefas", "Tarefas"], ["reunioes", "Reuniões"], ["lembretes", "Lembretes"], ["tudo", "Tudo"]];
    const n = filterCount();
    const bar = `<div class="calbar">` +
        ...
        `</div><span class="push"></span>` +
        `<div class="rangeseg">` + filters.map(([k, l]) => `<button data-calfilter="${k}"${state.calFilter === k ? ' aria-current="true"' : ""}>${l}</button>`).join("") +
        `</div>` +
    ...
    const main = state.calView === "dia" ? dayView() :
...
window.setInterval(() => {
    ...
    if (el)
        el.textContent = clock(state.elapsed);
    const big = document.getElementById("bigTimer");
    if (big)
        big.style.setProperty("--p", String(Math.min(100, (state.elapsed / (6 * 3600)) * 100)));
}, 1000);
...
document.addEventListener("click", e => {
    const el = e.target;
    const screenBtn = el.closest("[data-screen]");
    if (screenBtn) {
        setScreen(screenBtn.dataset["screen"]);
        return;
    }
    const timerBtn = el.closest("[data-task]");
    if (timerBtn) {
        toggleTimer(timerBtn.dataset["task"]);
        return;
    }
    const openBtn = el.closest("[data-open]");
    if (openBtn) {
        openTask(openBtn.dataset["open"]);
        return;
    }
    ...
    const mt = el.closest("[data-meeting]");
    if (mt) {
        openMeeting(mt.dataset["meeting"]);
        return;
    }
    const em = el.closest("[data-editmeeting]");
    if (em) {
        const m = MEETINGS.find(x => x.id === em.dataset["editmeeting"]);
        closeLayer();
        if (m)
            openMeetingForm(m);
        return;
    }
    const dm = el.closest("[data-delmeeting]");
    if (dm) {
        deleteMeeting(dm.dataset["delmeeting"]);
        return;
    }
    if (el.closest("[data-join]")) {
        closeLayer();
        toast("Abrindo a chamada…");
        return;
    }
    if (el.closest("[data-copymeeting]")) {
        closeLayer();
        toast("Link da reunião copiado.");
        return;
    }
    const chat = el.closest("[data-chat]");
    if (chat) {
        toast(`Conversa aberta com ${PEOPLE[chat.dataset["chat"]].name}.`);
        return;
    }
    const cd = el.closest("[data-calday]");
    if (cd) {
        state.calDate = new Date(cd.dataset["calday"] + "T00:00:00");
        if (state.calView !== "dia") {
            state.calView = "dia";
            renderCalHead();
        }
        renderCalendar();
    ...
    if (el.closest("[data-newreminder]")) {
        toast("Lembrete criado para hoje às 17:00.");
        return;
    }
    if (el.closest("[data-newchat]")) {
        toast("Conversa iniciada com o time do projeto.");
        return;
    }
...
let POP_WORKSPACES = [];
let POP_COMPANY_ID = "";
function preferencesKey(companyId = POP_COMPANY_ID) {
    return `pop-organize:v2-state:${companyId}`;
}
function readPopPreferences(companyId) {
    try {
        const value = JSON.parse(localStorage.getItem(preferencesKey(companyId)) ?? "{}");
...
function savePopPreferences() {
    if (!popBridgeActive || !POP_COMPANY_ID)
        return;
    try {

...
function isVisible(t) {
    ...
    if (state.query) {
        ...
        if (!`${t.title} ${t.desc} ${t.tag} ${sectorName}`.toLowerCase().includes(q))
            return false;
    }
    return true;
}
...
function matchesFilters(t) {
    ...
    if (state.who.size && !t.who.some(k => state.who.has(k)))
        return false;
    if (state.query && !`${t.title} ${t.desc} ${t.tag}`.toLowerCase().includes(state.query.toLowerCase()))
        return false;
    return true;
}
...
function openSectionForm() {
    const s = section();
    openLayer(`<div class="scrim"><div class="sheet" role="dialog" aria-modal="true" aria-label="Editar frente">` +
        ...
        $("#secForm").addEventListener("submit", e => {
            e.preventDefault();
            s.name = $("#sN").value.trim() || s.name;
            s.desc = $("#sD").value.trim();

...
// null until mounted so SSR/first paint still renders both trees (CSS picks the right one, no hydration flash)
function useTaskListLayout(): "desktop" | "mobile" | null {
        ...
        <div className="flex flex-col gap-3 lg:hidden">
          <AnimatePresence initial={false}>
            {tasks.map((task, index) => {
              ...
              const isMobileDropTarget =
                mobileDrag?.hasMoved && mobileDrag.beforeTaskId === task.id;
              return (
                <motion.div
                  key={task.id}
